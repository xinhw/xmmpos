...
public class DishEntityDao extends AbstractDao<DishEntity, Long> {
    ...
    @Override
    protected final void attachEntity(DishEntity entity) {
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getDishTypeEntityDao().getAllColumns());
            builder.append(" FROM DISH_ENTITY T");
            builder.append(" LEFT JOIN DISH_TYPE_ENTITY T0 ON T.\"TYPE_ID\"=T0.\"_id\"");
            builder.append(' ');
    ...
    protected DishEntity loadCurrentDeep(Cursor cursor, boolean lock) {
        DishEntity entity = loadCurrent(cursor, 0, lock);
        int offset = getAllColumns().length;

        DishTypeEntity dishType = loadCurrentOther(daoSession.getDishTypeEntityDao(), cursor, offset);
         if(dishType != null) {
    ...
    public DishEntity loadDeep(Long key) {
        assertSinglePk();
        if (key == null) {
            return null;
        }
    ...
    public List<DishEntity> loadAllDeepFromCursor(Cursor cursor) {
        int count = cursor.getCount();
        List<DishEntity> list = new ArrayList<DishEntity>(count);
    ...
    protected List<DishEntity> loadDeepAllAndCloseCursor(Cursor cursor) {
        try {
    ...
    public List<DishEntity> queryDeep(String where, String... selectionArg) {
